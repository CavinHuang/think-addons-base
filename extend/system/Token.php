...
class Token
{
    ...
    public static function connect(array $options = [], $name = false)
    {
        ...
        if (!isset($options['connection'])) {
            $options['connection'] = self::driverOptions($options, $type);
        }
        if (true === $name || !isset(self::$instance[$name])) {
            $class = false === strpos($type, '\\') ?
                '\\tyCore\\token\\driver\\' . ucwords($type) :
                $type;
     ...
     *
     */
    protected static function driverOptions($options, $type)
    {
