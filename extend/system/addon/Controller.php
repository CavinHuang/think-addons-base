...
class Controller extends \think\Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        $var = isset($dispatch->param) ? $dispatch->getParam() : [];
        $var = array_merge($param, $var);
        if (isset($dispatch->getParam()['method']) && substr($dispatch->getParam()['method'][0], 0, 7) == "\\addons")
        {
            $arr = explode("\\", $dispatch->getParam()['method'][0]);
            $addon = strtolower($arr[2]);
            $controller = strtolower(end($arr));
            $action = $dispatch->getParam()['method'][1];
        }
        else
        {
    ...
    {
        // 渲染配置到视图中
        $config = AddonTools::getAddonConfig($this->addon);
